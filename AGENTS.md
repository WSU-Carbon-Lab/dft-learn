...
- When those outputs exist, `dftrun postprocess` writes `packaged_output/stobe_final_energies.csv` (one row per site, energies in eV) and a single `packaged_output/stobe_orbital_energy_summary.png` (SCF totals, alpha-level dumbbells vs ground, and Delta-KS shifts for all sites). Totals come from `FINAL ENERGY` blocks; TP alpha LUMO (eV) comes from `ORBITAL ENERGIES (ALL VIRTUALS INCLUDED)` in `*tp.out`. The Delta-KS alignment shift is `E^c = E^e - E^g - E^l` with SCF totals converted Ha to eV and `E^l` the TP LUMO eigenvalue in eV.
