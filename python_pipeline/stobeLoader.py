...
def makeTDMTensor(j, i, maxRot, normalized_osx, normalized_osy, normalized_osz):
    """
    ...
    Returns:
    --------
    dict : Dictionary containing the 9 tensor elements (xx, xy, xz, yx, yy, yz, zx, zy, zz)
    """
...
def load_data(directory, width1, width2, max_energy, maxRot=4, verbose=True, n_jobs=None):
    """
    ...
    if not xray_transitions.empty and all(col in xray_transitions.columns for col in ['normalized_osx', 'normalized_osy', 'normalized_osz']):
        if verbose:
        ...
        if verbose:
            print(f"TDM tensor calculation completed for {len(xray_transitions)} transitions")
    
    elif not xray_transitions.empty:
        print("Warning: Required columns (normalized_osx, normalized_osy, normalized_osz) not found in xray_transitions")
