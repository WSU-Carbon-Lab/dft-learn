...
class NEXAFSAnalyzer:
    ...
    def fit_cluster_gaussians(self, energy_range=None, n_points=2000):
        ...
        Now includes tensor component aggregation by summing components for all peaks in each cluster.
        ...
        cluster_gaussians : pandas.DataFrame
            DataFrame with fitted Gaussian parameters and aggregated tensor components for each cluster
        """
        ...
        # Identify tensor component columns (common names for tensor components)
        tensor_columns = []
        possible_tensor_names = ['xx', 'xy', 'xz', 'yx','yy', 'yz', 'zx','zy','zz']
        
        for col in self.cluster_info.columns:
            if col in possible_tensor_names:
                tensor_columns.append(col)
        
        print(f"Found tensor component columns: {tensor_columns}")
        ...
        def calculate_fit_metrics(observed, predicted, threshold_fraction=1e-4):
            """
            Calculate fit quality metrics using scikit-learn
            ...
            predicted : array-like
                Predicted/fitted data points
            threshold_fraction : float
                Fraction of max signal to use as threshold for filtering noise
            ...
            # Filter out low-intensity points to avoid noise domination
            threshold = np.max(observed) * threshold_fraction
            ...
            mse = mean_squared_error(observed_masked, predicted_masked)
            
            # Calculate R-squared using scikit-learn
            r_squared = r2_score(observed_masked, predicted_masked)
            
            return mse, r_squared
        
        def calculate_theta_from_tensor(xx, yy, zz):
            """
            Calculate theta angle from diagonal tensor components.
            ...
            --------
            theta : float
                Angle in degrees between tensor vector (xx, yy, zz) and z-axis (0, 0, 1)
            """
            # Create tensor vector from diagonal components
            tensor_vector = np.array([xx, yy, zz])
            z_axis = np.array([0, 0, 1])
            
            # Calculate vector magnitude
            tensor_magnitude = np.linalg.norm(tensor_vector)
            
            # Handle zero magnitude case
            if tensor_magnitude == 0:
                return np.nan
            
            # Calculate dot product
            dot_product = np.dot(tensor_vector, z_axis)
            
            # Calculate angle using arccos, ensuring we stay within valid range [-1, 1]
            cos_theta = np.clip(dot_product / tensor_magnitude, -1.0, 1.0)
        ...
        for cluster_id in range(n_clusters):
            ...
            # Aggregate tensor components by summing
            tensor_sums = {}
            for tensor_col in tensor_columns:
                if tensor_col in cluster_peaks.columns:
                    tensor_sums[tensor_col] = cluster_peaks[tensor_col].sum()
                else:
                    tensor_sums[tensor_col] = 0.0
            
            # Calculate theta from summed diagonal tensor components
            if all(comp in tensor_sums for comp in ['xx', 'yy', 'zz']):
                cluster_theta = calculate_theta_from_tensor(
                    tensor_sums['xx'], 
                    tensor_sums['yy'], 
                    tensor_sums['zz']
                )
            else:
                # Fallback to weighted average if diagonal components not available
                if cluster_peaks['OS'].sum() > 0:
                    cluster_theta = np.average(cluster_peaks['theta'], weights=cluster_peaks['OS'])
                else:
                    cluster_theta = cluster_peaks['theta'].mean()
                print(f"Warning: Diagonal tensor components not found for cluster {cluster_id}, using weighted average theta")
            ...
            try:
                ...
                # Generate fitted curve for quality metrics calculation
                fitted_curve = gaussian_for_fitting(energy_axis, fitted_amplitude, fitted_center, fitted_sigma)
                
                # Calculate fit quality metrics using scikit-learn
                mse, r_squared = calculate_fit_metrics(cluster_intensity, fitted_curve)
                ...
                # Create cluster result dictionary
                cluster_result = {
                ...
                # Add theta if it exists
                cluster_result['theta'] = cluster_theta
                
                # Add tensor components to the result
                cluster_result.update(tensor_sums)
                
                cluster_gaussians.append(cluster_result)
                ...
                fallback_result['theta'] = cluster_theta

                # Add tensor components to the fallback result
                fallback_result.update(tensor_sums)
                
                cluster_gaussians.append(fallback_result)
        ...
        # Print tensor component summary
        if tensor_columns:
            print(f"Aggregated tensor components: {tensor_columns}")
            for tensor_col in tensor_columns:
                if tensor_col in cluster_gaussians_df.columns:
                    total_tensor = cluster_gaussians_df[tensor_col].sum()
                    original_total = self.cluster_info[tensor_col].sum() if tensor_col in self.cluster_info.columns else 0
                    print(f"  {tensor_col}: {original_total:.6f} → {total_tensor:.6f} (conservation check)")
        
        # Print fit quality summary
        successful_fits = cluster_gaussians_df[cluster_gaussians_df['fit_success']]
        if len(successful_fits) > 0:
            print(f"Fit quality statistics:")
            print(f"  MSE - Mean: {successful_fits['mse'].mean():.6f}, Median: {successful_fits['mse'].median():.6f}")
            print(f"  MSE - Min: {successful_fits['mse'].min():.6f}, Max: {successful_fits['mse'].max():.6f}")
            print(f"  R² - Mean: {successful_fits['r_squared'].mean():.4f}, Median: {successful_fits['r_squared'].median():.4f}")
            print(f"  R² - Min: {successful_fits['r_squared'].min():.4f}, Max: {successful_fits['r_squared'].max():.4f}")
            
            # Flag potentially poor fits based on R-squared
            poor_fits = successful_fits[successful_fits['r_squared'] < 0.9]
            if len(poor_fits) > 0:
                print(f"Warning: {len(poor_fits)} clusters have R² < 0.9 (potentially poor fits)")
            
            # Also flag fits with very high MSE (relative to signal)
            max_signal = successful_fits['height_at_center'].max()
            high_mse_threshold = (0.01 * max_signal) ** 2  # 1% of max signal squared
            high_mse_fits = successful_fits[successful_fits['mse'] > high_mse_threshold]
            if len(high_mse_fits) > 0:
                print(f"Warning: {len(high_mse_fits)} clusters have high MSE relative to signal strength")
    ...
    def iterative_analysis(self, overlap_threshold=50, method='sequential', n_skipped=1, 
        ...
        for iteration in range(max_iterations):
            ...
            current_peaks = cluster_gaussians[['E', 'width', 'OS','xx','xy','xz','yx','yy','yz','zx','zy','zz','theta']].copy()
