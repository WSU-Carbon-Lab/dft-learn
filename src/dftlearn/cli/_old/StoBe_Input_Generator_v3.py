...
import polystyrene_rotated.molConfig as molConfig
...
# pull out the environment variable locations for StoBe and the basis set files
...
def makeRunFile(
    ...
    for i in range(1, nAtoms + 1):
        ...
        if elem1_MCPbasis != "":
            with open(str(aname) + str(i) + ftype + ".run", "r") as f:
                with open(
                    str(aname) + str(i) + ftype + ".run", "a", newline="\n"
                ) as f1:
                    for line in f.readlines():
                        if n1 < nElem1:
                            line = elem1_MCPbasis + "\n"
...
def makeXASrun(mname, aname, nAtoms, title):
    ...
    if aname == "C" or "c":
        Erange = "280 320"
        Ebroad = "0.6 12 288 320"
    elif aname == "N" or "n":
        Erange = "400 450"
        Ebroad = "0.6 12 418 450 "
    elif aname == "O" or "o":
        Erange = "530 580"
        Ebroad = "0.6 12 535 560"
    elif aname == "Ni" or "ni":
        Erange = "830 880"
        Ebroad = "0.6 12 830 880"
    else:
        print(
            "The energy range and broadening has not been implemented for this element yet."
        )
    ...
    for i in range(1, nAtoms + 1):
        ...
        f.write("/.\n")
        f.write(
            f"{XASRUN} <" + aname + str(i) + "xas.inp>& " + aname + str(i) + "xas.out\n"
        )
...
def listFiles(
    aname, nAtoms
):  # This function organizes the .run files for all atoms into folders
    iniPath = os.getcwd()
    runfiles = []
    folders = []
...
def makeAllRunFiles():
    ...
    print("\n")
    print(
        "Please define the variable values to be used to build the files in a file called molConfig.py"
    )
    print("The molConfig.py file should be in the same directory as this python code.")
    print(
        "Ensure that the .xyz file containing the molecular geometry is in the current working directory!"
    )
    print("Important preliminary considerations for the .xyz file.")
    print("1.  The xyz file provided should be sorted by element.")
    print(
        "2.  The first group of elements in the file should be the element that we are calculating a NEXAFS for."
    )
    print(
        "3.  There should be a total of 4 columns in the xyz file corresponding to the atom label, and x, y, z coordinates."
    )
    print(
        "For example, if calculating the Carbon-edge NEXAFS for Benzene (C6H6), the first 6 atoms in the xyz file should be Carbon atoms labeled as C1,C2,C3,C4,C5,C6"
    )
    print("\n")
    ...
    mname = (
        molConfig.mname
    )  # This is the name of the molecule we are going to calculate
    aname = (
        molConfig.aname
    )  # This is the name of the element for which we are doing calculations
    nFiles = (
        molConfig.nFiles
    )  # This is the number of atoms for which we are generating input files
    difElems = (
        molConfig.difElems
    )  # This is the number of different ELEMENTS that are present in the molecule
    ...
    element2 = (
        molConfig.element2
    )  # This is the effective nuclear charge for the atoms of element2
    element3 = (
        molConfig.element3
    )  # This is the effective nuclear charge for the atoms of element3
    element4 = (
        molConfig.element4
    )  # This is the effective nuclear charge for the atoms of element4
    element5 = (
        molConfig.element5
    )  # This is the effective nuclear charge for the atoms of element5
    element6 = (
        molConfig.element6
    )  # This is the effective nuclear charge for the atoms of element6
    nElem1 = (
        molConfig.nElem1
    )  # This is the number of atoms of element1 present in the molecule
    nElem2 = (
        molConfig.nElem2
    )  # This is the number of atoms of element2 present in the molecule
    nElem3 = (
        molConfig.nElem3
    )  # This is the number of atoms of element3 present in the molecule
    nElem4 = (
        molConfig.nElem4
    )  # This is the number of atoms of element4 present in the molecule
    nElem5 = (
        molConfig.nElem5
    )  # This is the number of atoms of element5 present in the molecule
    nElem6 = (
        molConfig.nElem6
    )  # This is the number of atoms of element6 present in the molecule
    ...
    elem2_Abasis = (
        molConfig.elem2_Abasis
    )  # This is the auxiliary basis set for the atoms of element2
    elem3_Abasis = (
        molConfig.elem3_Abasis
    )  # This is the auxiliary basis set for the atoms of element3
    elem4_Abasis = (
        molConfig.elem4_Abasis
    )  # This is the auxiliary basis set for the atoms of element4
    elem5_Abasis = (
        molConfig.elem5_Abasis
    )  # This is the auxiliary basis set for the atoms of element5
    elem6_Abasis = (
        molConfig.elem6_Abasis
    )  # This is the auxiliary basis set for the atoms of element6
    ...
    elem2_Obasis = (
        molConfig.elem2_Obasis
    )  # This is the orbital basis set for the atoms of element2
    elem3_Obasis = (
        molConfig.elem3_Obasis
    )  # This is the orbital basis set for the atoms of element3
    elem4_Obasis = (
        molConfig.elem4_Obasis
    )  # This is the orbital basis set for the atoms of element4
    elem5_Obasis = (
        molConfig.elem5_Obasis
    )  # This is the orbital basis set for the atoms of element5
    elem6_Obasis = (
        molConfig.elem6_Obasis
    )  # This is the orbital basis set for the atoms of element6
    elem1_MCPbasis = (
        molConfig.elem1_MCPbasis
    )  # This is the model core potential basis set for the atoms of element1
    ...
    alfaOcc = (
        molConfig.alfaOcc
    )  # This is the electron occupation of the alpha orbitals for the excited state
    ...
    # copy folders to the mname directory

    # cwd = os.getcwd() # This line was in the original selection but is not used in the loop below.
    for i in range(1, nFiles + 1):
        source_folder_name = f"{aname}{i}"
        # destination_folder_path is the path where source_folder_name will reside inside mname
        destination_folder_path = os.path.join(mname, source_folder_name)

        # Check if the source folder (e.g., "C1") exists in the current directory and is a directory
        if os.path.isdir(source_folder_name):
            # If the target path (e.g., "MyMolecule/C1") already exists, remove it to ensure overwrite
            if os.path.exists(destination_folder_path):
                if os.path.isdir(destination_folder_path):
                    shutil.rmtree(destination_folder_path)
                else:  # It's a file, not a directory, at the destination path
                    os.remove(destination_folder_path)

            # Move the source folder (e.g., "C1") into the mname directory (e.g., "MyMolecule")
            # shutil.move(src, dst_dir) moves src into dst_dir, creating dst_dir/src_basename
            shutil.move(source_folder_name, mname)
        else:
            # This handles the case where the source folder aname{i} does not exist or is not a directory.
            print(
                f"Warning: Source '{source_folder_name}' is not a directory or does not exist in '{os.getcwd()}'. Skipping move."
            )
